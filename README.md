...
An small dashboard for Flashforge printers that contains a NodeJS server fetching printer data and serving to the client and a static Vue3 client (served statically by the server) (showing "realtime" data and the printer's webcam) in a single package.  
...
### TODO
- Refactor the client so it's less ugly, more componentized and maybe looks like something from the Klipper world (Mainsail my love <3).
